...
class mysiteSkinGenerator extends mysiteOptionGenerator {
	...
	function styles_dir() {
		if( is_multisite() ) {
			global $blog_id;
			if( $blog_id != 1 )
				$this->stylesdir = $_SERVER['DOCUMENT_ROOT'] . '/' . get_blog_option( $blog_id, 'upload_path' ) . '/styles';
			else
	...
	public function get_contents( $filename ) {
	    ...
	    if( array_key_exists( $filename, mysite_wpmu_style_option() ) ) {
	        global $blog_id;
	        $uri = get_blog_option( $blog_id,'fileupload_url') . '/styles/' . md5( THEME_NAME ) . 'muskin_' . $filename;
			$dir = $_SERVER['DOCUMENT_ROOT'] . '/' . get_blog_option( $blog_id, 'upload_path' ) . '/styles/' . md5( THEME_NAME ) . 'muskin_' . $filename;
	    } else {
	...
	public static function skin_upload() {
		...
		if( is_multisite() ) {
			global $blog_id;
			if( $blog_id != 1 )
				$stylesdir = $_SERVER['DOCUMENT_ROOT'] . '/' . get_blog_option( $blog_id, 'upload_path' ) . '/styles';
			else
