	...
	function nivo( $atts = null, $content = null ) {
		...
		if ( !preg_match_all( '/(.?)\[(image)\b(.*?)(?:(\/))?\](?:(.+?)\[\/image\])?(.?)/s', $content, $matches ) ) {
			...
			if( preg_match_all( '!https?://.+\.(?:jpe?g|png|gif)!Ui', $content, $matches ) ) {
				...
				$out .= '<img src="' . esc_url( THEME_IMAGES_ASSETS . '/transparent.gif' ) . '" style="background-image: url(' . THEME_IMAGES_ASSETS . '/loader_large.png);">';
				$out .= '</div>';
				...
				$out .= '</div><!-- #nivo_slider_## -->';
				$out .= '</div><!-- .nivo_load -->';
				$out .= '</div><!-- #mysite_nivo_sc_## -->';
			}
